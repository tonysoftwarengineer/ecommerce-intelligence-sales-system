...
def _base_cleaned():
    return {
        ...
        "order_items": pd.DataFrame({"order_id": ["o1"], "product_id": ["p1"], "price": [10.0]}),
        "customers": pd.DataFrame(
            {"customer_id": ["c1"], "customer_unique_id": ["u1"], "customer_state": ["SP"]}
        ),
        "products": pd.DataFrame({"product_id": ["p1"], "product_category_name": ["cat1"]}),
...
def test_join_all_raises_on_duplicate_order_id():
    ...
    cleaned["orders"] = pd.DataFrame({"order_id": ["o1", "o1"], "customer_id": ["c1", "c1"]})
